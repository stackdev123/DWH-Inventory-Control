...
import { Scan, ArrowDownLeft, CheckCircle, Camera, RefreshCw, X, Trash2, AlertCircle, Package, Layers, Zap } from 'lucide-react';
    ...
    <div className="flex flex-col gap-3 animate-in fade-in duration-300">
        ...
        {mode === 'scan' ? (
            ...
            <div className="space-y-4">
                ...
                <SearchableSelect 
                    ...
                    options={stock.filter(s => s.productId === selectedProductId && s.status === ItemStatus.CREATED).map(s => ({ id: s.uniqueId, name: `${s.batchCode} (${s.quantity})` }))} 
      ...
      <div className="space-y-3 max-h-[40vh] overflow-y-auto custom-scrollbar">
        {pendingQueue.map((item) => (
            <div key={item.uniqueId} className="bg-white p-4 rounded-[2rem] border border-slate-200 flex items-center justify-between shadow-sm">
                <div className="flex items-center gap-4">
                    ...
                    <div className="flex flex-col">
                        <span className="text-[11px] font-black text-slate-900 uppercase truncate max-w-[150px]">{item.productName}</span>
                        <span className="text-[9px] font-mono text-slate-400">BATCH: {item.batchCode}</span>
                    </div>
                </div>
                <div className="flex items-center gap-4">
